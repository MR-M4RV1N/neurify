...
class ProfileController extends AbstractController
{
    ...
    public function index(Request $request, ManagerRegistry $doctrine, SluggerInterface $slugger, TranslatorInterface $translator): Response
    {
        ...
        if (!$user) {
            $this->addFlash('error', $translator->trans('You must be logged in to edit your profile.'));
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($imageFile) {
                ...
                try {
                    ...
                    if ($originalImage && $originalImage !== 'default-user.jpg' && $originalImage !== 'default.jpg') {
            ...
            $this->addFlash('success', $translator->trans('Your profile was successfully updated.'));
        ...
        return $this->render('cpanel/profile/index.html.twig', [
            ...
            'myLevel' => $doctrine->getRepository(Level::class)->findOneBy([
                'lang' => $user->getLang(),
                'number' => ceil($user->getProgress() / 10) ?: 1
            ]),
        ]);
    }
